...
from integration.decoding.types import PixelFormat, CommandType
from integration.decoding.utils import (send_to_shared_memory, blocking_retry_send)
...
@dataclass
class PacketToVideoContext(ProcessorContext):
    container_buffer: BytesIO
    container_context: OutputContainer

class PacketToVideo(MessageProcessor):
    @staticmethod
    ...
    def init_session(self, message: ProcessMessage):
        ...
        session_context = PacketToVideoContext(
            command_type=CommandType.packet_to_frames_and_video,
            container_buffer=container_buffer,
            container_context=container_context,
        )

        self._logger.info(f"{message.session_id}|PtV|Init session")
    ...
    def clear_session(self, session_id: str, rack: Optional[str] = None, auto_clear: Optional[bool] = False):
        self._logger.info(f"{session_id}|PtV|Clear session")
        session_data: PacketToVideoContext = self._sessions[session_id]
    ...
    def process_message(self, message: ProcessMessage):
        ...
        if session_id is None:
            raise Exception("Session must specified for PtV command processing")
